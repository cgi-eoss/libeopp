...
import com.cgi.eoss.eopp.testing.docker.DockerClientRule;
...
public class RegistryContainerTest {
    ...
    @ClassRule(order = 0)
    public static DockerClientRule dockerClient = new DockerClientRule(true);

    @ClassRule(order = 1)
    public static RegistryContainer dockerRegistry = new RegistryContainer();
    ...
    public void test() throws Exception {
        ...
        assertThat(registryRepository).isEqualTo(dockerRegistry.getHost() + ":" + dockerRegistry.getHttpPort() + "/" + imageName);

        String imageNameWithRegistry = String.format("%s:%s/%s", dockerRegistry.getHost(), dockerRegistry.getHttpPort(), imageName);
