...
DEFAULT_POM_VERSION = "{pom_version}"
...
def _pom_file_impl(ctx):
    ...
    pom_version = coordinates.version
    substitutions = {
        "{groupId}": coordinates.groupId,
        "{artifactId}": coordinates.artifactId,
        "{version}": ctx.var.get("pom_version", "LOCAL-SNAPSHOT") if pom_version == DEFAULT_POM_VERSION else pom_version,
        "{type}": coordinates.type or "jar",
        "{scope}": coordinates.scope or "compile",
        "{artifactName}": ctx.attr.artifact_name or coordinates.artifactId,
    }
...
""",
    attrs = {
        "pom_template": attr.label(
            ...
            default = "//tools:pom-template.xml",
            allow_single_file = True,
        ),
        "artifact_name": attr.string(
            doc = "Maven artifact name",
            mandatory = False,
        ),
        "target": attr.label(
