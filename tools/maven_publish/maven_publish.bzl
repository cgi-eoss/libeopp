...
def _maven_publish_impl(ctx):
    ...
    if ctx.var.get("pom_version", "") != "":
        coordinates = coordinates.replace("{pom_version}", ctx.var.get("pom_version"))
...
""",
    ...
    attrs = {
        ...
        "_uploader": attr.label(
            executable = True,
            cfg = "exec",
            default = "@rules_jvm_external//private/tools/java/com/github/bazelbuild/rules_jvm_external/maven:MavenPublisher",
            allow_files = True,
        ),
    },
)
